...
NumSource =[20 40 60 80 100 120 140 160 180];            %声源的个数
NumB=30;                           %信标节点
...
%             UL2=round(rand(NumU/2,2)*500);        %未知节点
 ...
 %% %%%%%%%%%%% 未知节点对声源和信标节点的DOA测量值%%%%%%%%%%%%%%%%%%%%
            ...
            for ij=1:NumU
                ...
                if ku>=3                                
      %% %%%%%%%%%%%%%%%%%用已知的声源对未知节点进行定位BML   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    [OV,P_Bias, H_Bias]= PLS(SBStore,DOA,Unodes(1:2),Unodes(3));
...
Sigma=[20 40 60 80 100 120 140 160 180];
%% 数据存储
xlswrite('AV_PE',PBIAS);%位置误差保存
xlswrite('AV_HE',HBIAS);%角度误差保存
...
legend('Bearing Noise=pi/180','Bearing Noise=pi/90','Bearing Noise=pi/60');
xlabel('Bearing Noise Standard Deviation (degree)');
ylabel('Location Error (m)');
xlim([20 180])
grid on 
