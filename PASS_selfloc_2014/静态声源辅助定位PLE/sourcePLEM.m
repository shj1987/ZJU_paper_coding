...
Sigma=1:err;
%% 数据存储
xlswrite('LPLE_PE210',PBIAS);%位置误差保存
xlswrite('LPLE_HE210',HBIAS);%角度误差保存
% xlswrite('SKU',SKU);%角度误差保存
% % xlswrite('BML_Pvar',PVAR);
% % xlswrite('BML_Hvar',HVAR);
% xlswrite('LPort_PLE',Prt);

%% 图形显示1  位置节点平均误差
% figure(1)
% subplot(2,1,1)
% plot(Sigma,PBIAS(1,:),'b*--', Sigma,PBIAS(2,:),'kd--', Sigma,PBIAS(3,:),'r^--','linewidth',1.5)
...
% 
% subplot(2,1,2)
% plot(Sigma, HBIAS(1,:),'b*--', Sigma, HBIAS(2,:),'kd--', Sigma, HBIAS(3,:),'r^--','linewidth',1.5)
% %plot(Sigma, HBIAS(1,:),'b*--', Sigma, HBIAS(2,:),'kd--', Sigma, HBIAS(3,:),'r^--',Sigma, HBIAS(4,:),'go--','linewidth',1.6)
% set(gca,'Fontsize',15)
% legend('K=10','K=30','K=70');
% xlabel('Bearing Noise Standard Deviation (degree)');
% ylabel('Orientation Error (degree)')
% xlim([1 err])
% grid on 
% %% 图形显示2  节点方差
% figure(1)
...
%%  声源的平均误差
% figure(4)
% plot(Sigma, SBIAs(1,:),'b*--', Sigma, SBIAs(2,:),'kd--', Sigma, SBIAs(3,:),'r^--','linewidth',1.6)
% set(gca,'Fontsize',17)
% legend('M=40','M=50','M=6');
% xlabel('DOA误差，单位：°')
% ylabel('位置误差，单位：m')
% xlim([1 err])
% grid on 
