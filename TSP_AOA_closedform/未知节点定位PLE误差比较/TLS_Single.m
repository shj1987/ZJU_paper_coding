...
lt=length(Heading);
 if lt==2 &&((rem(abs(Heading(1)-Heading(2)),pi)<=0.35)||(rem(abs(Heading(1)-Heading(2)-pi),pi)<=0.35)) %防止共线情况
        XY=[0 0];
         ...
         XY=[0 0];
     else if lt==4&&((rem(abs(Heading(1)-Heading(2)),pi)<=0.3)||(rem(abs(Heading(1)-Heading(2)-pi),pi)<=0.3))&&((rem(abs(Heading(3)-Heading(2)),pi)<=0.3)||(rem(abs(Heading(3)-Heading(2)-pi),pi)<=0.3))&&((rem(abs(Heading(4)-Heading(3)),pi)<=0.3)||(rem(abs(Heading(4)-Heading(3)-pi),pi)<=0.3))
           XY=[0 0];
           display('********************++++++++++++++++++')
         else
              for i=1:lt
