...
%           Hdr=round(rand(1,1)*100*pi/2)/100;
 ...
 %% %%%%%%%%%%% 未知节点对声源和信标节点的DOA测量值%%%%%%%%%%%%%%%%%%%%
  ...
  %% 用已知的声源对未知节点进行定位
                ...
                if fg==1
                   [RT,PLE_PBias, PLE_HBias] = RPLS(SBStore,DOA,Unodes(1:2),Unodes(3));
                ...
                if  PLE_PBias< threshold && MP<60 && MH<40
                    num= num + 1;
...
set(h,'Fontsize',12)
xlabel('Orientation of Unknown Node (degree)');
