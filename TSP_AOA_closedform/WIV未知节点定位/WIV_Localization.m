...
z=0;
 ...
 %% %%%%%%%%%%% 未知节点对声源和信标节点的DOA测量值%%%%%%%%%%%%%%%%%%%%
  ...
  %% 用已知的声源对未知节点进行定位
                ...
                [UBC,B_xyz,BP_Bias, BH_Bias] = BCPLS(SBStore,DOA,Unodes(1:2),Unodes(3));%使用Bias Compensated PseudoLinear Least Squares.
...
figure(1)
subplot(2,1,1)
plot(Sigma, PBIAS,'b*--',Sigma, BC_PBIAS,'go--',Sigma, WIV_PBIAS,'rs--','linewidth',1.5)
set(gca,'Fontsize',14);
legend('PLE','BCPLE','WIV');
...
subplot(2,1,2)
plot(Sigma, HBIAS,'b*--',Sigma, BC_HBIAS,'go--',Sigma, WIV_HBIAS,'rs--','linewidth',1.5)
set(gca,'Fontsize',14)
legend('PLE','BCPLE','WIV');
xlabel('Bearing Noise Standard Deviation (degree)');
