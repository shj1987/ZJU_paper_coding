...
z=0;
 ...
 %% %%%%%%%%%%% 未知节点对声源和信标节点的DOA测量值%%%%%%%%%%%%%%%%%%%%
           S_Bcon=Beacon(:,1:2); 
            ...
            if ku>=3
  %% 用已知的声源对未知节点进行定位
                [UT,PLE_PBias, PLE_HBias] = PLS(SBStore,DOA,Unodes(1:2),Unodes(3)); %PLE
                ...
                if  PLE_PBias< threshold && MP<150 && PLE_WP< threshold
                    num= num + 1;
...
Sigma = 1:err;
%% 数据存储
PB=[PLE_P;PBIAS;PLE_WIVP;WIV_PB];%ple, bcple, avplewiv, bcplvwiv
HB=[PLE_H;HBIAS;PLE_WIVH;WIV_HB];
xlswrite('PB100', PB);        %位置误差保存
xlswrite('PH100', HB);        %角度误差保存
...
plot(Sigma,PLE_P,'m>--',Sigma,PBIAS,'b*--', Sigma, WIV_PB,'g+--',Sigma, PLE_WIVP,'ro--','linewidth',1.5)
set(gca,'Fontsize',13);
...
plot(Sigma,PLE_H,'m>--',Sigma,HBIAS,'b*--', Sigma, WIV_HB,'g+--',Sigma, PLE_WIVH,'ro--','linewidth',1.5)
set(gca,'Fontsize',13)
%legend('PLE','BCPLE','ML','BCAVPLE-WIV','AVPLE-WIV');
h=legend('AVPLE','BCAVPLE','BCAVPLE-WIV','AVPLE-WIV');
set(h,'Fontsize',12);
